(function () {
  ...
  function handleContinue(stepNum) {
    // Validate only this step's own fields before ever revealing the next
    // step. An empty or invalid active step must never be pushed forward —
    // that would distribute its errors into a newly-revealed context the
    // user has not yet reached. Useful focus (the first invalid control)
    // is retained on this step instead of advancing.
    var fieldIds = STEP_FIELDS[stepNum] || [];
    var errors = computeErrors(fieldIds);
    applyStatesFor(fieldIds, errors);
    if (errors.length) {
      renderErrorSummary(errors);
      setStatus("This step has " + errors.length + " problem" +
        (errors.length === 1 ? "" : "s") + " that must be fixed before you can continue. " +
        errors[0].message);
      focusTarget(errors[0].fieldId);
      persistState();
      return;
    }
    clearErrorSummary();
  ...
  function wireEvents() {
    ...
    if (skipLink) {
      // Move programmatic (not just visual) focus to the checkout steps.
      // The href="#wizard-form" + tabindex="-1" fallback still works with
      // JavaScript disabled; this handler additionally targets the current
      // active step's own heading so screen-reader users land inside the
      // step they are about to work on, not merely at the form boundary.
      skipLink.addEventListener("click", function (event) {
        event.preventDefault();
        var activeHeading = document.querySelector('.step[data-step="' + state.currentFocus + '"] h2');
        var target = activeHeading || byId("wizard-form");
    ...
    var clearConfirm = byId("clear-confirm");
    var cancelClear = document.querySelector('[data-action="cancel-clear"]');
    var confirmClear = document.querySelector('[data-action="confirm-clear"]');
    // Clear saved progress is a destructive, irreversible action (it wipes
    // localStorage and reloads). Require an explicit second confirmation
    // step — with an equally prominent Cancel — before it discards state,
    // rather than acting immediately on the first activation.
    if (clearProgress && clearConfirm) {
      clearProgress.addEventListener("click", function () {
        ...
        setStatus("Clear saved progress needs confirmation. Choose \u201cYes, clear saved progress\u201d " +
          "or Cancel below.");
        if (cancelClear) cancelClear.focus();
      });
    }
    ...
    if (clearConfirm) {
      clearConfirm.addEventListener("keydown", function (event) {
