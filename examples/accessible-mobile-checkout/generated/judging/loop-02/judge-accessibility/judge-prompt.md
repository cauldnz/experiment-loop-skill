...
No other judge notes or identity map were read.
...
```text
You are the approved independent blind `judge-accessibility` for Loop 02 of the frozen accessibility-first mobile checkout Experiment. Operate and judge three blinded candidates. Do not edit candidates, read Track/model identities, read the orchestrator-only blinding map, read other judge output, or create a new candidate.
...
1. `.experiments\accessible-mobile-checkout\generated\judging\blind-loop-02\candidate-a`
2. `.experiments\accessible-mobile-checkout\generated\judging\blind-loop-02\candidate-b`
3. `.experiments\accessible-mobile-checkout\generated\judging\blind-loop-02\candidate-c`
...
Write scope ONLY:
`.experiments\accessible-mobile-checkout\generated\judging\loop-02\judge-accessibility\**`
...
- `candidate-a.json/.md`, `candidate-b.json/.md`, `candidate-c.json/.md` with actual model `gpt-5.6-terra`, exact scores, evidence refs relative to generated root, all 11 lens findings, visual-information-clarity, strengths, defects, uncertainty, and one evidence-backed synthesis adoption/rejection recommendation;
...
- `pairwise.json/.md` comparing A/B, B/C, A/C in that order, preference, rationale, confidence, and evidence refs, without guessing identity;
